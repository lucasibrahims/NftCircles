...
async function mint () {
    if(ethereum) {
        try{
            ...
            let value = 5*10**15;
            let call = await contratoNFT.methods.mint().send({from:account, value:value});
            return call;
...
mintBnt.addEventListener('click', () => {
    mint().then((response) => {
    }).catch((err) => {
         console.log(err);
     });
